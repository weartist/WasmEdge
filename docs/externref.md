...
To instantiate the above example Wasm, the host functions must be registered into WasmEdge. See [Host Functions](https://github.com/WasmEdge/WasmEdge/blob/master/docs/host_function.md) for more details.
...
if (Result) {
  ...
  std::cout << Returns.first[0].get<uint32_t>(); // will print 3409269
  // The `Returns.second[0]` must be `WasmEdge::ValType::I32` because the `call_mul` returns an i32 value.
}
```
