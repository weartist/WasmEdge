...
bool SpecTest::compare(const std::pair<std::string, std::string> &Expected,
  ...
  if (!IsV128 && ValStr.substr(0, 4) == "nan:"sv) {
    ...
    if (TypeStr == "f32"sv) {
      ...
      return std::isnan(Got.get<float>());
    } else if (TypeStr == "f64"sv) {
      if (GotT != ValType::F64) {
        return false;
      }
      return std::isnan(Got.get<double>());
    }
  } else if (TypeStr == "funcref"sv) {
    if (GotT != ValType::FuncRef) {
      return false;
    }
    if (ValStr == "null"sv) {
      return WasmEdge::isNullRef(Got);
    } else {
      if (WasmEdge::isNullRef(Got)) {
        return false;
      }
      return WasmEdge::retrieveFuncIdx(Got) ==
             static_cast<uint32_t>(std::stoul(ValStr));
    }
  } else if (TypeStr == "externref"sv) {
    if (GotT != ValType::ExternRef) {
      return false;
    }
    if (ValStr == "null"sv) {
      return WasmEdge::isNullRef(Got);
      ...
      return static_cast<uint32_t>(reinterpret_cast<uintptr_t>(
                 &WasmEdge::retrieveExternRef<uint32_t>(Got))) ==
             static_cast<uint32_t>(std::stoul(ValStr));
    }
    ...
    return Got.get<uint32_t>() == uint32_t(std::stoul(ValStr));
    ...
    return Got.get<uint64_t>() == uint64_t(std::stoull(ValStr));
  } else if (TypeStr == "f64"sv) {
    if (GotT != ValType::F64) {
      return false;
    }
    ...
    if (LaneType == "f32") {
      ...
      const uint64x2_t V64 = {
          static_cast<uint64_t>(Got.get<uint128_t>()),
          static_cast<uint64_t>(Got.get<uint128_t>() >> 64U)};
      ...
      using uint64x2_t [[gnu::vector_size(16)]] = uint64_t;
      const uint64x2_t V = {static_cast<uint64_t>(Got.get<uint128_t>()),
                            static_cast<uint64_t>(Got.get<uint128_t>() >> 64U)};
      for (size_t I = 0; I < 2; ++I) {
...
bool SpecTest::compares(
  ...
  if (Expected.size() != Got.size() || Expected.size() != GotT.size()) {
    return false;
  }
  for (size_t I = 0; I < Expected.size(); ++I) {
    if (!compare(Expected[I], Got[I], GotT[I])) {
      return false;
    }
  }
  return true;
...
void SpecTest::run(std::string_view Proposal, std::string_view UnitName) {
  ...
  auto Invoke = [&](const rapidjson::Value &Action,
    ...
    if (auto Res = onInvoke(ModName, Field, Params.first, Params.second)) {
      /// Check value.
      EXPECT_TRUE(compares(Returns, (*Res).first, (*Res).second));
    } else {
  ...
  auto Get = [&](const rapidjson::Value &Action,
    ...
    if (auto Res = onGet(ModName, Field)) {
      /// Check value.
      EXPECT_TRUE(compare(Returns[0], (*Res).first, (*Res).second));
    } else {
      EXPECT_NE(LineNumber, LineNumber);
    }
  };
